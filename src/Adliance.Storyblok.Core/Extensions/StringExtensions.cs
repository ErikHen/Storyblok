...
namespace Adliance.Storyblok.Core.Extensions
{
    ...
    {
        public static bool IsStoryblokEditorUrl(this string pathAndQuery)
        {
            var query = HttpUtility.ParseQueryString(pathAndQuery);
            ...
            var validationString = $"{query["_storyblok_tk[space_id]"]}:{StoryblokOptions.ApiKeyPreview}:{query["_storyblok_tk[timestamp]"]}";
